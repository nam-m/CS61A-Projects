...
def roll_dice(num_rolls,dice=six_sided): #changed from dice=six_sided to dice
    ...
    while i <= num_rolls:
        d = dice()
        # print ("Result of this dice is", d)
        if d == 1:
            # print("Pig Out!")
            sum = 1
        if sum != 1:
...
def free_bacon(score):
    ...
    last_digit = score % 10
    min_digit = min(first_digit, last_digit)
    return 10-min_digit
...
def is_swap(player_score, opponent_score):
    ...
    if the first (leftmost) digit of the current player's score 
    and the last (rightmost) digit of the opponent player's score are equal, 
    ...
    while first_digit_player > 9:
        first_digit_player = first_digit_player // 10

    if first_digit_player == opponent_score % 10:
        # print ("Swine Swap!")
        return True
    return False 
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        #In case 1 of the player reaches goal first
        if is_swap(score0, score1) == True:
            score0, score1 = score1, score0

        # BEGIN PROBLEM 6   
        #Call commentary function at the end of first turn and call return value in the next turns
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        #Workaround for UnboundLocalError
        # nonlocal allows modifying variable defined in outer function from inner functions
        # variable has to be defined before
...
# fixed_dice = make_test_dice(2,41,1,54)
# print(roll_dice(4,dice=fixed_dice))
...
# s0, s1 = play(always(0), always(0), score0=9, score1=92, dice=always_three)
# print(s0,s1)
# print(is_swap(17,101))

# Handle multiple turns with many swaps
# s0, s1 = play(always(1), always(1), goal=30, dice=make_test_dice(6, 1, 4, 6, 5, 2, 9))
# print(s0,s1)
